// questions.js - Baza pytan dla gry Milionerzy
// Tematyka: Fizyka klasa 7 - Cisnienie, gestosc, sily

const FALLBACK_QUESTIONS = [
    // === 1. Sily spojnosci i przylegania, napiecie powierzchniowe ===
    {
        id: 1,
    ...
    // === 3. Wyznaczanie gestosci przedmiotow ===
    {
        id: 10,
    ...
    // === 11. Zaokraglanie do cyfr znaczacych ===
    {
        id: 40,
...
// Mutowalna tablica pytan - domyslnie kopia fallback
let Questions = [...FALLBACK_QUESTIONS];

// Ustaw nowe pytania (z Gemini AI)
function setQuestions(newQuestions) {
    Questions = newQuestions;
}

// Przywroc domyslne pytania fizyki
function resetToFallbackQuestions() {
    Questions = [...FALLBACK_QUESTIONS];
}

// Zaladuj pytania z cache (localStorage) - class-aware
function loadCachedQuestions() {
    try {
        const activeClassId = Storage.getActiveClass() || 'default_fizyka7';

        if (activeClassId === 'default_fizyka7') {
            Questions = [...FALLBACK_QUESTIONS];
            console.log('[Milionerzy] Aktywna klasa: domyslna fizyka (65 pytan)');
            return true;
        }
        ...
        if (cached && Array.isArray(cached) && cached.length > 0) {
            Questions = cached;
            console.log('[Milionerzy] Zaladowano ' + cached.length + ' pytan dla klasy: ' + activeClassId);
            return true;
        }

        // Fallback if class questions not found
        console.log('[Milionerzy] Brak pytan dla klasy ' + activeClassId + ' - uzywam domyslnych');
        Questions = [...FALLBACK_QUESTIONS];
        return false;
    } catch (e) {
        console.warn('[Milionerzy] Nie udalo sie zaladowac pytan z cache:', e);
        Questions = [...FALLBACK_QUESTIONS];
        return false;
    }
}

// Zaladuj pytania dla konkretnej klasy
function loadQuestionsForClass(classId) {
...
// Auto-restore z cache przy ladowaniu skryptu
loadCachedQuestions();

// Funkcja do losowania pytan
function getRandomQuestions(count = 10) {
    const shuffled = [...Questions].sort(() => Math.random() - 0.5);
    return shuffled.slice(0, count);
}

// Funkcja do mieszania odpowiedzi
function shuffleAnswers(question) {
    // Nie mieszaj odpowiedzi dla pytan prawda/falsz
...
// === FUNKCJE POMOCNICZE DLA KATEGORII ===
...
function getUniqueCategories() {
    ...
    Questions.forEach(q => {
        if (q.category) {
            const mainCat = getMainCategory(q.category);
...
// Pobierz glowna kategorie (uproszczenie)
function getMainCategory(category) {
    // Jesli aktywna klasa to nie domyslna fizyka, zwroc kategorie bez mapowania
    const activeClassId = Storage.getActiveClass() || 'default_fizyka7';
    if (activeClassId !== 'default_fizyka7') {
        return category;
    }
    const categoryLower = category.toLowerCase();
    if (categoryLower.includes('gestosc')) return 'Gestosc';
    if (categoryLower.includes('cisnienie')) return 'Cisnienie';
    if (categoryLower.includes('archimedes') || categoryLower.includes('wypor') || categoryLower.includes('plywanie')) return 'Archimedes';
    if (categoryLower.includes('napiecie') || categoryLower.includes('spojnosc') || categoryLower.includes('przylegania')) return 'Napiecie powierzchniowe';
    if (categoryLower.includes('pascal')) return 'Prawo Pascala';
    if (categoryLower.includes('jednostki') || categoryLower.includes('cyfry')) return 'Jednostki';
    return category;
}

// Pobierz pytania z okreslonej kategorii
function getQuestionsByCategory(categoryName) {
    return Questions.filter(q => {
        if (!q.category) return false;
        return getMainCategory(q.category) === categoryName;
    });
}

// Pobierz losowe pytania z kategorii
function getRandomQuestionsFromCategory(categoryName, count = 10) {
    const categoryQuestions = getQuestionsByCategory(categoryName);
    const shuffled = [...categoryQuestions].sort(() => Math.random() - 0.5);
    return shuffled.slice(0, Math.min(count, shuffled.length));
}

// Pobierz trudnosc pytania (1-3 na podstawie kategorii lub pola difficulty)
function getQuestionDifficulty(question) {
    // Jesli pytanie ma pole difficulty (wygenerowane przez AI), uzyj go
    if (question.difficulty) {
        return Math.min(3, Math.max(1, question.difficulty));
    }

    const category = getMainCategory(question.category || '');

    // Proste tematy
    if (['Jednostki', 'Gestosc'].includes(category)) {
        return 1;
    }

    // Srednie tematy
    if (['Cisnienie', 'Napiecie powierzchniowe'].includes(category)) {
        return 2;
    }

    // Trudne tematy
    if (['Archimedes', 'Prawo Pascala'].includes(category)) {
        return 3;
    }

    return 2; // domyslnie srednie
}
